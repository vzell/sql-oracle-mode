;;; sql-oracle-transform.el --- transform SQL statements

;; Copyright (C) 1998-2000  Alexander Schroeder
;; Copyright (C) 2017  Dr. Volker Zell <vzell@volkerzell.de>
...
;; sql-oracle-transform.el is free software; you can redistribute it and/or
;; modify it under the terms of the GNU General Public License as
;; published by the Free Software Foundation; either version 2, or (at
;; your option) any later version.

;; sql-oracle-transform.el is distributed in the hope that it will be useful,
...
;; 04.11.2017 - vzell - Added group by, having and order by support, plus Oracle specific ";" and "/"
...
(defvar sql-transform-keyword-regexp "[ \t\n]*\\(from\\|into\\|where\\|group\\|having\\|order\\|set\\)\\b"
...
(defvar sql-end-where-regexp "\\b\\(group\\|order\\)\\b"
  "Regexp matching the end of a WHERE clause.
A WHERE clause is also ended by `sql-statement-end-regexp'.")

(defvar sql-end-having-regexp "\\border\\b"
  "Regexp matching the end of a HAVING clause.
A HAVING clause is also ended by `sql-statement-end-regexp'.")
...
(defun sql-max-length-list-elements (list)
  "Return maximum length of strings in list."
  (let ((length)
	(max-length))
    (setq max-length (length (car list)))
    (setq list (cdr list))
    (while list
      (setq length (length (car list)))
...
`sql-statement-find-start' and `sql-statement-find-end'."
  ...
  (let* ((start (sql-statement-find-start))
	 ...
	 (target-column)
	 (max-column-name-length (number-to-string (sql-max-length-list-elements columns))))
    (goto-char start)
    (setq target-column (current-column))
    (kill-region start end)
    (insert "update    " table "\nset\n")
    (insert (format (concat "\t  %" max-column-name-length "s = %s") (car columns) (car bindvars)))
...
`sql-statement-find-start' and `sql-statement-find-end'."
  ...
  (let* ((start (sql-statement-find-start))
    ...
    (indent-to target-column)
    (if no-newlines
	(insert (format "values ( %s" (car bindvars)))
...
`sql-statement-find-start' and `sql-statement-find-end'."
  ...
  (let* ((start (sql-statement-find-start))
	 ...
	 (columns (nth 2 statement))
	 (bindvars (nth 3 statement))
	 (groupbycolumns (nth 4 statement))
	 (having (nth 5 statement))
	 (orderbycolumns (nth 6 statement))
	 (no-newlines nil)
;	 (no-newlines (< (length columns) 6))
	 (target-column))
    (goto-char start)
	...
	(progn
	  (newline 1)
	  (insert "group by\n")
	  (insert (format "\t  %s" (car groupbycolumns)))
	  (setq groupbycolumns (cdr groupbycolumns))
	  (while groupbycolumns
	    (newline 1)
	    (indent-to target-column)
	    (insert "\t, ")
	    (insert (car groupbycolumns))
	    (setq groupbycolumns (cdr groupbycolumns)))
	  (indent-to target-column)))
    (if having
	(progn
	  (newline 1)
	  (indent-to target-column)
	  (sql-insert-having-clause having target-column)))
    (if orderbycolumns
	(progn
	  (newline 1)
	  (insert "order by\n")
	  (insert (format "\t  %s" (car orderbycolumns)))
	  (setq orderbycolumns (cdr orderbycolumns))
	  (while orderbycolumns
	    (newline 1)
	    (indent-to target-column)
	    (insert "\t, ")
	    (insert (car orderbycolumns))
	    (setq orderbycolumns (cdr orderbycolumns)))
	  (indent-to target-column)))
...
;    (insert "where")
	...
	(indent-to target-column))
      (setq line (car lines)
           lines (cdr lines))
      ;; FIXME: collapse whitespace inside string
      (if (string-match "^\\(and\\|or\\) " line)
	  (progn
	    (let ((lo (downcase (match-string 1 line))))
	      (if (string-equal lo "and") 
;		  (insert "  " lo "\n")
;		(insert "   " lo "\n"))) ;; this is "or"
		  (insert "  " lo)
		(insert "   " lo))) ;; this is "or"
	    (insert (format "\t  %s" (replace-match "" nil nil line))))
...
(defun sql-insert-having-clause (lines target-column)
  "Insert HAVING clause at point.
First parameter LINES is a list of lines, second parameter
TARGET-COLUMN is the target column at which the HAVING string
should start."
  (let ((line)
	(markers)
	(first t))
...
See `sql-parse-statement' for more information."
  (let ((columns)
    ...
    ;; group by clause: get everything after the GROUP BY keyword.
    (goto-char start)
    (if (sql-find-regexp "[ \t\n]*\\bgroup[ \t\n]+by\\b" end)
	(setq group-by (sql-parse-csv (point) end)))
    ;; having clause: get everything after the HAVING keyword.
    (goto-char start)
    (if (sql-find-regexp "[ \t\n]*\\bhaving\\b" end)
	(setq having (sql-parse-having (point) end)))
    ;; order by clause: get everything after the ORDER BY keyword.
    (goto-char start)
    (if (sql-find-regexp "[ \t\n]*\\border[ \t\n]+by\\b" end)
	(setq order-by (sql-parse-csv (point) end)))
    (list tables where columns bindvars group-by having order-by)))
...
outside a string matches `sql-transform-keyword-regexp' or END is reached."
  (goto-char start)
  (let ((words)
	(pos))
    (while (and (not (looking-at sql-transform-keyword-regexp))
		(not (looking-at "[ \t\n]*)"))
		(not (looking-at "\n[ \t\n]*/\\|;")) ;; v.z.: otherwise the next forward-sexp statement jumps over /
		(<= (point) end))
      (forward-sexp)
      (if (or (looking-at "[ \t\n]*[,)]")
	      (looking-at sql-transform-keyword-regexp)
...
when a word outside a string matches `sql-transform-keyword-regexp' or END is
reached."
  (goto-char start)
  (let* ((columns)
	 (bindvars)
...
(defun sql-parse-having (start end)
  "Parse a HAVING clause into a list of lines.
The region between START and END is parsed.  A new line is started
whenever `sql-insert-newline-regexp' or
`sql-insert-newline-and-indent-regexp' matches.  
Whitespace is collapsed.  Leading whitespace is stripped."
  ...
  (let ((lines)
    ...
    (while (and (not done)
      ...
      (cond (;; collapse whitespace
	    ...
	    ((or (looking-at sql-end-having-regexp)
		 (= (point) end))
	     (setq done t)); leave loop
	    (t; skip forward across strings (we jump into parens in a previous cond)
	     (forward-sexp))))
...
(provide 'sql-oracle-transform)

;;; sql-oracle-transform.el ends here
